...
# Fungsi route untuk halaman home dokter dan admin
# @app.route('/home')
# def home():
...
def loginadmin():
    ...
    if request.method == 'POST':
        ...
        print(f"Akun Karyawan ditemukan: {user}")

        if user.password and check_password_hash(user.password, password):
            print("Login berhasil!")
            session['username'] = username
            session['hak_akses'] = user.hak_akses  # Menyimpan informasi hak akses dalam session
            flash('Login berhasil!', 'success')
            
            # Arahkan ke halaman yang sesuai dengan hak akses
            if user.hak_akses == 'admin':
                return redirect(url_for('home_admin'))
            elif user.hak_akses == 'dokter':
...
def add_karyawan():
    if request.method == 'POST':
        ...
        if not nik or not username or not password or not hak_akses:
        ...
        try:
            ...
            flash(f'Gagal menambahkan karyawan. Kesalahan: {str(e)}', 'danger')
